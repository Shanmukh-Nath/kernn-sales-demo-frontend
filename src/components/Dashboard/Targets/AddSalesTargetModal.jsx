...
function AddSalesTargetModal({ changeTrigger }) {
  ...
  const validateFields = () => {
    ...
    if (!targetQuantity) newErrors.targetQuantity = true;
  ...
  const onSubmitClick = () => {
    ...
    async function create() {
      try {
        ...
        setSuccessful(res.data.message || "Sales target created successfully");
        changeTrigger();
        ...
        console.log(e);
        setError(e.response?.data?.message || "Failed to create sales target");
        setIsModalOpen(true);
      } finally {
        setLoading(false);
      }
    ...
    <>
      <DialogRoot placement={"center"} size={"lg"} className={styles.mdl}>
        <DialogTrigger asChild>
          <button className="homebtn">+ Add Sales Target</button>
        </DialogTrigger>
        <DialogContent className="mdl">
          <DialogBody>
            <h3 className={`px-3 pb-3 mdl-title`}>Create Sales Target</h3>
            ...
            <div className="row justify-content-center">
              <div className={`col-4 inputcolumn-mdl`}>
                ...
                <input
                  ...
                  onChange={(e) => onError(e, targetQuantity, setTargetQuantity)}
                  required
                  className={errors.targetQuantity ? styles.errorField : ""}
...
export default AddSalesTargetModal;

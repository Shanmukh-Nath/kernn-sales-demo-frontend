...
import AddCustomerTargetModal from "./AddCustomerTargetModal";

function CustomerTargets({ navigate, isAdmin }) {
  ...
  const fetchTargets = async () => {
    try {
      ...
      setError("Failed to fetch customer targets");
      setIsModalOpen(true);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
      <p className="path">
        ...
        <i className="bi bi-chevron-right"></i> Customer Targets
      </p>

            {isAdmin && (
        <div className="row m-0 p-3 pt-0">
          <div className="col-3">
            <AddCustomerTargetModal changeTrigger={changeTrigger} />
          </div>
        </div>
      )}
          ...
          <div className="col-lg-10">
            <table className="table table-bordered borderedtable">
              ...
              <tbody>
                ...
                {targets.length > 0 &&
                  targets.map((target) => {
                    const progress = target.achievedCustomers ? 
                      Math.min((target.achievedCustomers / target.targetCustomers) * 100, 100) : 0;
